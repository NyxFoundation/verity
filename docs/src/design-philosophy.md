...
- **Treating panics as acceptable.** Relying on a global safety net to absorb crashes is incompatible with proving that the core cannot crash. Verity does not adopt that posture.
...
- **Optimization that trades away verifiability.** Performance matters, but not at the price of a core we can no longer reason about. Verity optimizes only behind the verification boundary or in ways that preserve the correspondence with the model, and only once a real bottleneck is shown.
