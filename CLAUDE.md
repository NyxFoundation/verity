...
- **Dependencies**:
  ...
  - **leanSig pins Plonky3 to nothing — `Cargo.lock` is load-bearing here.** leanSig's manifest declares `p3-* = { git = ".../Plonky3.git" }` with neither rev nor branch, so a fresh resolve floats to Plonky3's default-branch HEAD. That HEAD (v0.6.0) moved to `num-bigint` 0.5 while leanSig's code is written against 0.4, so leanSig stops compiling with no change on our side. The pin that holds it is `3f67d136c71bec40f180c85d0bb2b654acddef22` (v0.5.1) — the rev ethlambda locks against the same leanSig rev.
    - **The pin is not in `Cargo.lock` right now.** Retiring the day-one canary crate left no member depending on leanSig, and cargo prunes what nothing depends on: the `p3-*` entries, `leansig`, `rocksdb`, and `libp2p` all left the lockfile with it. Whoever first adds `leansig` back — `verity-crypto` — must re-establish the rev in the same change, with `cargo update p3-field --precise 3f67d136c71bec40f180c85d0bb2b654acddef22`, and confirm leanSig compiles before merging. Skipping it does not fail quietly; it fails on the first build.
